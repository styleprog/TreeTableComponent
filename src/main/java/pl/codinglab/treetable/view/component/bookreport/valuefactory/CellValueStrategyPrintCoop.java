...
public class CellValueStrategyPrintCoop implements CellValueStrategy {
    ...
    public ObservableValue<String> createValueFactory(TreeTableColumn.CellDataFeatures<TreeTableRow, String> param) {
        ...
        if (entireData.isPresent()) {
            return new ReadOnlyStringWrapper(entireData.get().getEntirePrintCoop());
        } else if (reportData.isPresent()) {
            return new ReadOnlyStringWrapper(reportData.get().getPrintCoop());
        }
