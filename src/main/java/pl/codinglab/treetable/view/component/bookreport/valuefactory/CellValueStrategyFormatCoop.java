...
public class CellValueStrategyFormatCoop implements CellValueStrategy {
    ...
    public ObservableValue<String> createValueFactory(TreeTableColumn.CellDataFeatures<TreeTableRow, String> param) {
        ...
        if (entireData.isPresent()) {
            return new ReadOnlyStringWrapper(entireData.get().getEntireFormatCoop());
        } else if (reportData.isPresent()) {
            return new ReadOnlyStringWrapper(reportData.get().getFormatCoop());
        }
