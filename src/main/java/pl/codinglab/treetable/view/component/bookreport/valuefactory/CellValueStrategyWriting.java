...
public class CellValueStrategyWriting implements CellValueStrategy {
    ...
    public ObservableValue<String> createValueFactory(TreeTableColumn.CellDataFeatures<TreeTableRow, String> param) {
        ...
        if (entireData.isPresent()) {
            return new ReadOnlyStringWrapper(entireData.get().getEntireWriting());
        } else if (reportData.isPresent()) {
            return new ReadOnlyStringWrapper(reportData.get().getWriting());
        }
